...
CREATE SEQUENCE "public"."seq_sys_parent_child_id" INCREMENT 1 START 1;
...
-- ==========================================
-- 表四：亲子绑定关系表 (sys_parent_child)
-- ==========================================
CREATE TABLE "public"."sys_parent_child" (
                                             "relation_id" int8 NOT NULL DEFAULT nextval('seq_sys_parent_child_id'::regclass),
                                             ...
                                             "create_time" timestamp(6) DEFAULT pg_systimestamp(),
                                             CONSTRAINT "pk_sys_parent_child" PRIMARY KEY ("relation_id")
)
    WITH (orientation=ROW)
    DISTRIBUTE BY HASH("parent_user_id");

-- 核心索引
CREATE UNIQUE INDEX "uk_parent_child" ON "public"."sys_parent_child" ("parent_user_id", "child_user_id");
CREATE INDEX "idx_child_parent" ON "public"."sys_parent_child" ("child_user_id");

-- 表及字段全注释
COMMENT ON TABLE "public"."sys_parent_child" IS '亲子绑定关系表';
COMMENT ON COLUMN "public"."sys_parent_child"."relation_id" IS '亲子绑定关系自增主键';
COMMENT ON COLUMN "public"."sys_parent_child"."parent_user_id" IS '家长的用户唯一UID (关联sys_user.user_id)';
COMMENT ON COLUMN "public"."sys_parent_child"."child_user_id" IS '孩子的用户唯一UID (关联sys_user.user_id)';
COMMENT ON COLUMN "public"."sys_parent_child"."relation_type" IS '监护关系类型 (1父亲, 2母亲, 3其他法定监护人)';
COMMENT ON COLUMN "public"."sys_parent_child"."auth_status" IS '绑定关系的审核/确认状态 (0待确认/审批中, 1已确认绑定成功, 2已拒绝解绑)';
COMMENT ON COLUMN "public"."sys_parent_child"."bind_time" IS '亲子关系确认绑定的生效时间';
COMMENT ON COLUMN "public"."sys_parent_child"."create_time" IS '家长发起绑定申请的时间';
