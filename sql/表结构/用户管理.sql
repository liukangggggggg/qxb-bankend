...
CREATE SEQUENCE "public"."seq_parent_child_relation_id" INCREMENT 1 START 1;
...
-- ==========================================
-- 表四：亲子绑定关系表 (parent_child_relation)
-- ==========================================
CREATE TABLE "public"."parent_child_relation" (
    "relation_id" int8 NOT NULL DEFAULT nextval('seq_parent_child_relation_id'::regclass),
    ...
    "create_time" timestamp(6) DEFAULT pg_systimestamp(),
    CONSTRAINT "pk_parent_child_relation" PRIMARY KEY ("relation_id")
)
    WITH (orientation=ROW)
    DISTRIBUTE BY HASH("parent_user_id");

-- 核心索引
CREATE UNIQUE INDEX "uk_parent_child" ON "public"."parent_child_relation" ("parent_user_id", "child_user_id");
CREATE INDEX "idx_child_parent" ON "public"."parent_child_relation" ("child_user_id");

-- 表及字段全注释
COMMENT ON TABLE "public"."parent_child_relation" IS '亲子绑定关系表';
COMMENT ON COLUMN "public"."parent_child_relation"."relation_id" IS '亲子绑定关系自增主键';
COMMENT ON COLUMN "public"."parent_child_relation"."parent_user_id" IS '家长的用户唯一UID (关联sys_user.user_id)';
COMMENT ON COLUMN "public"."parent_child_relation"."child_user_id" IS '孩子的用户唯一UID (关联sys_user.user_id)';
COMMENT ON COLUMN "public"."parent_child_relation"."relation_type" IS '监护关系类型 (1父亲, 2母亲, 3其他法定监护人)';
COMMENT ON COLUMN "public"."parent_child_relation"."auth_status" IS '绑定关系的审核/确认状态 (0待确认/审批中, 1已确认绑定成功, 2已拒绝解绑)';
COMMENT ON COLUMN "public"."parent_child_relation"."bind_time" IS '亲子关系确认绑定的生效时间';
COMMENT ON COLUMN "public"."parent_child_relation"."create_time" IS '家长发起绑定申请的时间';
