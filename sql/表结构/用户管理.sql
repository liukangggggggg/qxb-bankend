...
COMMENT ON COLUMN "public"."qxb_user"."real_name" IS '真实姓名 (高频查询字段，实名认证后写入)';
COMMENT ON COLUMN "public"."qxb_user"."avatar" IS '用户头像URL地址 (预留255位以兼容多端长URL)';
COMMENT ON COLUMN "public"."qxb_user"."email" IS '电子邮箱 (用于找回密码、通知推送)';
COMMENT ON COLUMN "public"."qxb_user"."city" IS '所在城市 (咨询师检索、本地服务推荐的核心筛选条件)';
...
COMMENT ON COLUMN "public"."qxb_user"."ext_info" IS '扩展属性JSON (低频杂项字段兜底，如mailing_address、id_no等，键值可自由扩展)';
...
COMMENT ON COLUMN "public"."sys_user_role"."user_id" IS '用户ID（关联qxb_user.user_id）';
COMMENT ON COLUMN "public"."sys_user_role"."role_id" IS '角色ID（关联sys_role.role_id）';


-- ==========================================
-- 表七：学生信息表 (qxb_student_info)
-- ==========================================
CREATE SEQUENCE "public"."seq_qxb_student_id" INCREMENT 1 START 1;

CREATE TABLE "public"."qxb_student_info" (
    "id" int8 NOT NULL DEFAULT nextval('seq_qxb_student_id'::regclass),
    "user_id" int8 NOT NULL,
    ...
    "create_time" timestamp(6) DEFAULT pg_systimestamp(),
    CONSTRAINT "pk_qxb_student_info" PRIMARY KEY ("id")
)
    WITH (orientation=ROW)
    DISTRIBUTE BY HASH("user_id");

CREATE UNIQUE INDEX "uk_student_no" ON "public"."qxb_student_info" ("student_no");
CREATE INDEX "idx_student_user" ON "public"."qxb_student_info" ("user_id");

COMMENT ON TABLE "public"."qxb_student_info" IS '学生信息表（绑定码认领模式）';
COMMENT ON COLUMN "public"."qxb_student_info"."id" IS '自增主键';
COMMENT ON COLUMN "public"."qxb_student_info"."user_id" IS '关联qxb_user.user_id';
COMMENT ON COLUMN "public"."qxb_student_info"."student_no" IS '学号（唯一标识）';
COMMENT ON COLUMN "public"."qxb_student_info"."class_name" IS '班级名称';
COMMENT ON COLUMN "public"."qxb_student_info"."grade" IS '年级';
COMMENT ON COLUMN "public"."qxb_student_info"."school_name" IS '学校名称';
COMMENT ON COLUMN "public"."qxb_student_info"."binding_code" IS '6位绑定码（家长认领时校验）';
COMMENT ON COLUMN "public"."qxb_student_info"."bind_status" IS '绑定状态（0未绑定 1已绑定）';
COMMENT ON COLUMN "public"."qxb_student_info"."create_time" IS '创建时间';
...
COMMENT ON COLUMN "public"."biz_counselor_profile"."user_id" IS '关联qxb_user.user_id';
...
COMMENT ON COLUMN "public"."biz_counselor_qualification"."user_id" IS '关联qxb_user.user_id';
